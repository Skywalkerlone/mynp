...
//   // New: Technical Portfolio & Web Development
//   technicalServices: {
...
//   // New: AI Training & Data Annotation
//   aiServices: {
...
//   // New: Computer Science & Programming Education
//   csEducation: {
...
//       // Also try loading after a delay (for some browsers)
...
//   // Speak text with soothing male voice - FIXED VERSION
...
//     // Force lower pitch for masculine voice (this works even without specific male voice)
//     utterance.pitch = 0.75 // Lower = deeper/more masculine
//     utterance.rate = 1
//     utterance.volume = 1
    
//     // Try to find a male voice
//     const voices = window.speechSynthesis.getVoices()
//     console.log('Available voices for speech:', voices.map(v => ({ name: v.name, lang: v.lang })))
    
//     // Comprehensive list of male voice names/patterns
...
//       if (voice) {
//         selectedVoice = voice
//         console.log('Found male voice (exact/partial):', voice.name)
//         break
//       }
//     }
    
//     // Second pass: check if voice name or lang indicates male
//     if (!selectedVoice) {
...
//       )
//       if (selectedVoice) {
//         console.log('Found male voice (keyword match):', selectedVoice.name)
//       }
//     }
    
//     // Third pass: use any English voice as fallback (but keep low pitch)
//     if (!selectedVoice) {
//       selectedVoice = voices.find(v => v.lang.startsWith('en'))
//       if (selectedVoice) {
//         console.log('Using English fallback voice:', selectedVoice.name)
//         // Lower pitch even more for fallback voices
//         utterance.pitch = 0.6
//       }
//     }
...
//       utterance.pitch = 0.5 // Drastically lower pitch
//     }
    
//     if (selectedVoice) {
//       utterance.voice = selectedVoice
//       console.log('Final voice selection:', selectedVoice.name, 'Pitch:', utterance.pitch)
//     } else {
//       console.warn('No voices available, using default voice with lowered pitch')
//     }
...
// Add to your geminiMockData in Chatbot.js
...
export default function Chatbot() {
  ...
  const [useNeuralNet, setUseNeuralNet] = useState(true) // Toggle between neural net and mock data
  const recognitionRef = useRef(null)
  const messagesEndRef = useRef(null)
  const inputRef = useRef(null)
  const { darkMode } = useTheme()
  ...
  const getReply = async (userMessage) => {
    ...
    // Try using neural network first if ready and enabled
  ...
  const handleSend = async (textOverride = null) => {
    ...
    try {
      // Get reply from neural network or mock data
